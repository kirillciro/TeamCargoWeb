...
export default function WhatsAppTab({ win98 = false }: { win98?: boolean }) {
  ...
  const fetchMessages = useCallback(
    async (year = filterYear, month = filterMonth, day = filterDay) => {
  ...
  const handleDisconnect = () =>
    showConfirm(
      ...
      async () => {
        ...
        try {
          await fetchWithAuth("/api/admin/whatsapp/disconnect", {
            method: "POST",
          });
          await fetchStatus();
        } finally {
  ...
  const handleAddRecipient = async () => {
    ...
    if (!PHONE_RE.test(num)) {
      setNumberError(
        "Enter a valid number with country code, e.g. +393497080551",
      );
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setNumberError(data.message ?? "Failed to add number");
        return;
      }
      setRecipients(data.numbers ?? []);
      setNewNumber("");
  ...
  const applyFilter = () =>
    void fetchMessages(filterYear, filterMonth, filterDay);
  const clearFilter = () => {
    setFilterYear("");
    setFilterMonth("");
    setFilterDay("");
        ...
        >
          ...
          >
            ...
            {waStatus?.status === "open" && waStatus.senderPhone && (
              <div style={{ fontSize: 12 }}>
                Sending from: <strong>{waStatus.senderPhone}</strong>
              </div>
            )}
            ...
            {qrDataUrl && waStatus?.status !== "open" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <div style={{ fontSize: 11, color: "#000080" }}>
            ...
            {waStatus?.status === "open" && (
              <>
                ...
                >
                  ✓ Connected. Cargo notifications will be sent from{" "}
                  {waStatus.senderPhone ?? "this account"}.
                </div>
                <button
            ...
            {waStatus?.status === "connecting" && !qrDataUrl && (
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <div style={{ fontSize: 12, color: "#808000" }}>
          ...
          >
            ...
            <div style={{ fontSize: 11, color: "#444" }}>
              Cargo messages are sent to all numbers below.
            </div>
            ...
            >
              <div style={{ display: "flex", gap: 6 }}>
                <input
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && void handleAddRecipient()
                  }
                ...
                <button
                  ...
                  style={{
                    ...
                    fontSize: 12,
                    ...W98_RAISED,
                    cursor: addingNumber ? "wait" : "pointer",
                    background: "#c0c0c0",
                  }}
                >
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              {(msg.recipients as string[]).length} rcpt
                            </span>
                          </div>
                          <button
                            onClick={async () => {
                              ...
                              await fetchWithAuth(
                                `/api/admin/whatsapp/messages/${msg.id}`,
                                { method: "DELETE" },
                              );
                              setDeletingIds((p) => {
                                const n = new Set(p);
        ...
        {confirmModal && (
          ...
          >
            ...
            >
              ...
              <div style={{ padding: "14px 16px 10px", fontSize: 12 }}>
              ...
              >
                <button
                  onClick={() => {
                    closeConfirm();
                    void (confirmModal.onConfirm as () => Promise<void>)();
                  }}
                  style={{
                    padding: "2px 16px",
                    fontSize: 12,
        ...
        <div className="w-full lg:w-100 shrink-0 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex items-start gap-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-4 py-3">
                  ...
                  <p className="text-sm text-emerald-300">
                    WhatsApp is connected. Use the{" "}
                    <span className="font-medium">Send WhatsApp</span> button on
                    any cargo extraction.
                  </p>
                </div>
                <button
                  onClick={() => void handleDisconnect()}
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="bg-white rounded-2xl p-3 sm:p-4 w-full">
                  ...
                  <img
                    src={qrDataUrl}
                    alt="WhatsApp QR Code"
                    className="block w-full h-auto max-w-85 sm:max-w-none mx-auto"
                    style={{ aspectRatio: "1 / 1" }}
                  />
                </div>
                <p className="text-xs text-slate-500">
                  QR refreshes automatically. Scan quickly — it expires every
                  ~20 s.
                </p>
              </div>
            )}
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 sm:p-7 flex flex-col gap-5">
            <div>
              ...
              <p className="text-slate-400 text-xs mt-0.5">
                Cargo notifications are sent to all numbers below
              </p>
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <p className="text-xs text-slate-600">
                Include country code, e.g. +393497080551 or +31644351451
              </p>
            </div>
          </div>
        </div>
            ...
            {messages.length === 0 ? (
              <div className="flex flex-col items-center gap-2 py-8 text-slate-600">
                ...
                <p className="text-sm">
                  {filterYear || filterMonth || filterDay
                    ? "No messages match the filter."
                    : "Messages will appear here after sending."}
                </p>
              </div>
            ) : (
              <>
                ...
                <ul className="flex flex-col gap-2 overflow-y-auto max-h-150 lg:max-h-[calc(100vh-22rem)]">
                  {messages.map((msg) => {
                    ...
                    return (
                      <li
                        key={msg.id}
                        className={`border rounded-lg overflow-hidden transition-colors ${selected ? "bg-amber-400/5 border-amber-400/30" : "bg-slate-800/60 border-slate-700"}`}
                      >
                          ...
                          <div className="border-t border-slate-700 px-4 py-3 flex flex-col gap-2">
                            <div className="flex flex-wrap gap-1.5">
                              {(msg.recipients as string[]).map((r) => (
                                <span
                                  key={r}
                                  className="text-xs font-mono bg-slate-700 text-slate-300 rounded px-2 py-0.5"
                                >
                                  {r}
                                </span>
                              ))}
                            ...
                            <pre className="mt-1 text-xs text-slate-300 whitespace-pre-wrap wrap-break-word font-mono bg-slate-900/60 rounded-lg px-3 py-2 max-h-64 overflow-y-auto">
                              {msg.message_text}
                            </pre>
                          </div>
                        )}
